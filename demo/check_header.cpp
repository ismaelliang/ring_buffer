...
void display_header_info(const RingBufferHeader *header) {
    ...
    for (uint32_t i = 0; i < header->num_consumers; ++i) {
        ...
        std::cout << "消费者 " << std::setw(2) << i << ": " << "tail=" << std::setw(4) << tail
                  << ", 待消费=" << std::setw(4) << queue_size << " 条消息" << std::endl;
    }

    std::cout << "\n=== 内存布局信息 ===" << std::endl;
    size_t header_size =
        sizeof(RingBufferHeader) + (header->num_consumers - 1) * sizeof(RingBufferHeader::ConsumerTail);
    ...
    std::cout << "总大小:     " << total_size << " 字节 (" << std::fixed << std::setprecision(2) << total_size / 1024.0
              << " KB)" << std::endl;
}
    ...
    try {
        ...
        if (fd == -1) {
            if (errno == ENOENT) {
                ...
                std::cerr << "错误: 无法打开共享内存 '" << shm_name << "': " << strerror(errno) << std::endl;
            }
            return 1;
        }
