  ...
  try {
    ...
    while (true) {
      if (total_message_count.has_value() &&
          messages_produced >= total_message_count.value()) {
        std::cout << "Produced " << messages_produced << " messages. Exiting."
                  << std::endl;
        break;
      }
      // Prepare sample data
      ...
      if (queue.publish(MessageType::MARKET_DATA, &data, sizeof(MarketData))) {
        std::cout << "Produced: " << data.symbol << " Price: " << data.price
                  << " Volume: " << data.volume
      ...
      std::this_thread::sleep_for(
          std::chrono::microseconds(1000)); // 1ms instead of 100ms
    }
