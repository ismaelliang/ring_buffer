...
struct LatencyStats {
  ...
  uint64_t getPercentile(double percentile) const {
    ...
    int index = static_cast<int>(
                    std::ceil(percentile / 100.0 * sorted_latencies.size())) -
                1;
    if (index < 0)
      index = 0; // Handle percentile 0 edge case
    if (static_cast<std::vector<uint64_t>::size_type>(index) >=
  ...
  void printStats() const {
    std::cout << "\n=== 延迟统计 ===\n"
              << "最小延迟: " << min_latency << "ns (" << min_latency / 1000.0
              << "μs)\n"
              << "最大延迟: " << max_latency << "ns (" << max_latency / 1000.0
              << "μs)\n"
              << "平均延迟: " << getAverageLatency() << "ns ("
              << getAverageLatency() / 1000.0 << "μs)\n"
              << "P50 延迟: " << getPercentile(50) << "ns ("
              << getPercentile(50) / 1000.0 << "μs)\n"
              << "P95 延迟: " << getPercentile(95) << "ns ("
              << getPercentile(95) / 1000.0 << "μs)\n"
              << "P99 延迟: " << getPercentile(99) << "ns ("
              << getPercentile(99) / 1000.0 << "μs)\n"
              << "样本数量: " << count << "\n"
              << std::endl;
  }
};
...
void print_usage(const char *program_name) {
  std::cerr << "Usage: " << program_name
            << " [-n <count>] [-c <num_consumers>] [-id <consumer_id>]"
            << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "-n" || arg == "--num") {
      ...
      if (i + 1 < argc) {
        ...
        std::cerr << "Error: -id/--consumer_id requires an argument."
                  << std::endl;
        print_usage(argv[0]);
        return 1;
      }
  ...
  if (consumer_id >= num_consumers) {
    std::cerr << "Error: consumer_id (" << consumer_id
              << ") must be less than num_consumers (" << num_consumers << ")."
              << std::endl;
    print_usage(argv[0]);
    return 1;
  }
  ...
  try {
    ...
    std::cout << "Available CPU cores: " << CPUAffinity::getCPUCount()
              << std::endl;
    int target_cpu =
        consumer_id + 1; // Start from CPU 1, assuming producer uses CPU 0
    if (target_cpu >= CPUAffinity::getCPUCount()) {
      std::cerr << "Error: Not enough CPU cores to bind consumer "
                << consumer_id << " to core " << target_cpu
                << ". Available cores: " << CPUAffinity::getCPUCount()
                << std::endl;
      return 1;
    }
    ...
    std::vector<char> received_message_buffer(sizeof(MessageHeader) +
    ...
    long long messages_consumed =
        0; // Use long long for potentially large counts
    int msg_count = 0;
    ...
    while (true) {
      if (total_message_count.has_value() &&
          messages_consumed >= total_message_count.value()) {
        std::cout << "Consumed " << messages_consumed << " messages. Exiting."
                  << std::endl;
        break;
      }

      // Try to read data from message queue
      if (queue.subscribe(received_message_buffer.data(), consumer_id)) {
        // Interpret the received buffer as a GenericMessage
        const GenericMessage *received_msg =
            reinterpret_cast<const GenericMessage *>(
                received_message_buffer.data());

        // Cast the payload to MarketData
        const MarketData *received_data =
            reinterpret_cast<const MarketData *>(received_msg->payload);
        ...
        std::cout << "Received: " << received_data->symbol
                  ...
                  << " Latency: " << latency << "ns (" << std::fixed
                  << std::setprecision(2) << latency / 1000.0 << "μs)"
                  << std::endl;
