 ...
 *     // 填充您的消息数据，包括消息类型
 *     // msg.header.type 和 msg.header.payload_size 将由 produce 方法自动填充
 *     msg.price = 100.0;
 *     msg.quantity = 10;
 *     strncpy(msg.symbol, "AAPL", 8);
 *
 *     // 3. 生产消息
 *     if (queue.produce(MessageType::MARKET_DATA, &msg.price,
 * sizeof(MyMarketDataMessage) - sizeof(MessageHeader))) { std::cout <<
 * "Producer: Produced market data." << std::endl; } else { std::cerr <<
 * "Producer: Queue is full, failed to produce." << std::endl;
 *     }
 * }
 * @endcode
 *
 ...
 *
 *     // 3. 消费消息
 *     if (queue.consume(recv_buffer.data(), 0)) {
...
class CPUAffinity {
  public:
    ...
    static bool bindToCPU(int cpu_id) {
        ...
        if (result != 0) {
            std::cerr << "Failed to bind to CPU " << cpu_id << ": " << strerror(errno) << std::endl;
            return false;
        }
    ...
    static bool setRealtimePriority(int priority = 99) {
        ...
        if (sched_setscheduler(0, SCHED_FIFO, &param) == -1) {
            std::cerr << "Failed to set real-time priority (需要root权限): " << strerror(errno) << std::endl;
            return false;
        }
...
class MmapRingBuffer {
  public:
     ...
     */
    static bool
    isHeaderCompatible(const char *name, uint32_t element_count, uint32_t element_size, uint32_t num_consumers) {
        // 尝试打开现有的共享内存
        ...
        // 检查头部字段是否匹配
        bool is_compatible = (header->size == element_count) && (header->element_size == element_size) &&
     ...
     * @param no_create 如果为 true，仅连接到现有的兼容共享内存，不创建新的；如果共享内存不存在或不兼容则抛出异常。
     */
    MmapRingBuffer(const char *name,
        ...
        if (no_create && force_recreate) {
            throw std::invalid_argument("no_create and force_recreate cannot both be true");
        }
  ...
  public:
    ...
    bool push(const void *data) {
        ...
        uint32_t min_consumer_tail = header_->consumer_tails[0].tail.load(std::memory_order_acquire);
        for (uint32_t i = 1; i < header_->num_consumers; ++i) {
            uint32_t consumer_tail = header_->consumer_tails[i].tail.load(std::memory_order_acquire);
        ...
        char *element_ptr = static_cast<char *>(buffer_) +
                            (sizeof(RingBufferHeader) + (num_consumers_ - 1) * sizeof(std::atomic<uint32_t>)) +
    ...
    bool pop(void *data, uint32_t consumer_id) {
        ...
        uint32_t current_tail = header_->consumer_tails[consumer_id].tail.load(std::memory_order_relaxed);
        ...
        // 计算读取位置。
        const char *element_ptr = static_cast<char *>(buffer_) +
                                  (sizeof(RingBufferHeader) + (num_consumers_ - 1) * sizeof(std::atomic<uint32_t>)) +
        ...
        header_->consumer_tails[consumer_id].tail.store(next_tail, std::memory_order_release);
    ...
    bool empty(uint32_t consumer_id) const {
        ...
        return header_->head.load(std::memory_order_acquire) ==
               header_->consumer_tails[consumer_id].tail.load(std::memory_order_acquire);
    }
    ...
    uint32_t current_size(uint32_t consumer_id) const {
        ...
        uint32_t current_head = header_->head.load(std::memory_order_acquire);
        uint32_t current_tail = header_->consumer_tails[consumer_id].tail.load(std::memory_order_acquire);
...
class MessageQueue {
  public:
     ...
     * @return 如果头部匹配，则返回 true；否则返回 false。
     */
    static bool isHeaderCompatible(const std::string &name,
        ...
        // 检查头部字段是否匹配
        bool is_compatible = (header->size == queue_capacity) && (header->element_size == expected_element_size) &&
                             (header->num_consumers == num_consumers);
    ...
    MessageQueue(const std::string &name,
                 ...
                 bool force_recreate = false,
                 bool no_create = false)
        : max_payload_size_(max_payload_size), total_message_size_(sizeof(MessageHeader) + max_payload_size),
          buffer_(name.c_str(),
                  queue_capacity,
                  total_message_size_,
                  num_consumers,
                  force_recreate,
                  no_create),                                  // Pass all parameters including no_create
          next_sequence_num_(0), num_consumers_(num_consumers) ///< 在此处初始化 `buffer_`。
    {
        if (max_payload_size_ == 0) {
            throw std::invalid_argument("max_payload_size cannot be zero.");
        }
        if (total_message_size_ > 0xFFFFFFFF) { ///< 检查 `total_message_size_` 是否溢出（如果它是
                                                ///< `uint32_t`）。
     ...
     * @return 如果消息成功生产，则为 `true`；如果队列已满，则为 `false`。
     */
    bool produce(MessageType type, const void *data, uint32_t data_size) {
     ...
     */
    bool consume(void *message_buffer, uint32_t consumer_id) {
        if (!message_buffer) {
            throw std::invalid_argument("Provided message_buffer is null.");
        }
        if (consumer_id >= num_consumers_) {
            throw std::out_of_range("consumer_id out of range for this MessageQueue instance.");
        }
        // 从底层环形缓冲区弹出完整消息。
