 ...
 * 首先，定义您的应用程序特定的消息结构。所有消息都应包含一个 `MessageHeader` 作为前缀。
 * 例如：
 * @code
 ...
 * void producer_main() {
 *     // 1. 创建或连接到消息队列
 *     // 参数: 队列名称, 队列容量 (消息数量), 最大负载大小 (单个消息有效载荷的最大字节数)
 *     MessageQueue queue("my_shared_queue", 1024, sizeof(MyMarketDataMessage) - sizeof(MessageHeader));
 *
 *     // 2. 准备消息
 ...
 *
 *     // 3. 发布消息
 *     if (queue.publish(MessageType::MARKET_DATA, &msg.price, sizeof(MyMarketDataMessage) - sizeof(MessageHeader))) {
 *         std::cout << "Producer: Published market data." << std::endl;
 *     } else {
 *         std::cerr << "Producer: Queue is full, failed to publish." << std::endl;
 *     }
 * }
 * @endcode
 *
 ...
 *     std::vector<char> recv_buffer(sizeof(MessageHeader) + queue.max_payload_size());
 *
 *     // 3. 订阅消息
 *     if (queue.subscribe(recv_buffer.data())) {
 *         // 4. 将接收到的通用消息转换为您的特定消息结构
 *         GenericMessage* generic_msg = reinterpret_cast<GenericMessage*>(recv_buffer.data());
 *         // 现在可以访问头部信息
 *         std::cout << "Consumer: Received message type: " << static_cast<uint32_t>(generic_msg->header.type)
 *                   << ", payload size: " << generic_msg->header.payload_size << std::endl;
 *
 *         // 如果消息类型是您的 MyMarketDataMessage，则可以进一步转换和访问其内容
 *         if (generic_msg->header.type == MessageType::MARKET_DATA) {
 *             MyMarketDataMessage* market_data = reinterpret_cast<MyMarketDataMessage*>(generic_msg->payload);
 *             // 注意: 实际的 payload 是从 GenericMessage.payload 开始的
 *             // 所以需要偏移 MessageHeader 的大小来获取实际的 MyMarketDataMessage 结构
 *             // 另一种方法是直接将 recv_buffer 转换为 MyMarketDataMessage*，但这需要确保 MyMarketDataMessage 的布局正确
 *             // 这里我们假设 MyMarketDataMessage 包含了 MessageHeader
 *             market_data = reinterpret_cast<MyMarketDataMessage*>(recv_buffer.data());
 *             std::cout << "  Market Data - Price: " << market_data->price
 ...
 * 例如，在 CMake 中，可以设置：
 * @code cmake
 * set(CMAKE_CXX_FLAGS_RELEASE "-O3 -march=native -mtune=native -DNDEBUG -flto -ffast-math -funroll-loops")
 ...
 * (`CPUAffinity::setRealtimePriority`) 可以在高负载或多核环境中进一步提升性能稳定性。
 */
#pragma once
 ...
 */
struct alignas(64) GenericMessage
{                         ///< 为提升性能，按缓存行对齐。
    MessageHeader header; ///< 消息头部。
    /**
     * @brief 实际消息数据的占位符。
     *        此数组的大小将在创建 `MessageQueue` 时由 `max_payload_size` 参数确定。
     *        这是一个柔性数组（flexible array member）概念，但对于固定大小消息，它只是一个缓冲区。
     */
...
public:
    ...
    bool push(const void *data)
    {
        ...
        // 计算写入位置。
        char *element_ptr = static_cast<char *>(buffer_) + sizeof(RingBufferHeader) + current_head * header_->element_size;
    ...
    bool pop(void *data)
    {
        ...
        // 计算读取位置。
        const char *element_ptr = static_cast<char *>(buffer_) + sizeof(RingBufferHeader) + current_tail * header_->element_size;
...
public:
     ...
     *                         底层环形缓冲区中实际的元素大小将是 `sizeof(MessageHeader) + max_payload_size`。
     */
    MessageQueue(const std::string &name, uint32_t queue_capacity, uint32_t max_payload_size)
        : max_payload_size_(max_payload_size),
          ...
          next_sequence_num_(0) ///< 在此处初始化 `buffer_`。
    {
        if (max_payload_size_ == 0)
        {
            throw std::invalid_argument("max_payload_size cannot be zero.");
        }
        if (total_message_size_ > 0xFFFFFFFF)
        { ///< 检查 `total_message_size_` 是否溢出（如果它是 `uint32_t`）。
     ...
     */
    bool subscribe(void *message_buffer)
    {
        if (!message_buffer)
        {
